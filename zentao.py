...
def xmind_to_zentao_csv_file(xmind_file):
    ...
    fileheader = ["所属模块", "用例标题","请求URL","请求方法","请求参数","用例等级","预期结果","执行结果"]
...
def gen_a_testcase_row(testcase_dict):
    ...
    case_title = testcase_dict['name'] #用例标题 
    case_url=testcase_dict['url']#接口的url
    case_method=testcase_dict['method'] #接口的方法
    case_precontion = testcase_dict['preconditions'] #请求参数
    case_priority = gen_case_priority(testcase_dict['importance']) #用例等级
    # case_step, case_expected_result = gen_case_step_and_expected_result(testcase_dict['steps'])
    case_expected_resut=x #预期结果
    case_result=x  #执行结果 
   
  
  
    row = [case_module, case_title, case_precontion, case_step, case_expected_result,case_priority]
    return row
